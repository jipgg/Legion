...
static void push_luau_module(lua_State* L, const fs::path& path, const char* as) {
    auto source = common::read_file(path);
    assert(source);
    std::string bytecode = Luau::compile(*source, copts());
    if (luau_load(L, as, bytecode.data(), bytecode.size(), 0)) {
        common::printerr("failed to load", as,"library:", bytecode);
    }
    lua_pcall(L, 0, 1, 0);
}
static void load_luau_module(lua_State* L, const fs::path& path, const char* as) {
    push_luau_module(L, path, as);
    lua_setglobal(L, as);
...
static void init_luau_state(lua_State* L, const fs::path& main_entry_point) {
    ...
    lua_pushcfunction(L, bi::is_key_down, "isKeyDown");
