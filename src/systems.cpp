...
void systems::physics(storage_view<ty::physical_component> components, double delta_s) {
    constexpr double big_mass{1e25};
    constexpr double gravity = -120.f;
    constexpr int x = 0;
    constexpr int y = 1;
    constexpr float deadzone = .1f;
    for (ty::physical_component& a : components) {
        ...
        for (ty::physical_component& b : components) {
            if (&a == &b) continue;
